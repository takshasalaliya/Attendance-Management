...
@endif       @foreach($datas as $data)
            ...
            <form action="final_attendent" method="post"> 
                ...
                <div class="col-md-6 mb-4 pb-2">
                  ...
                  <b style="font-size:20px; ">{{++$sum}} &nbsp;<label for="{{$sum}}">{{$student->name}}</label></b>
                  <input type="text" value="{{$student->student_id}}" name="student[]" hidden>
                ...
                <input type="radio" name="{{$student->student_id}}" value="absent" id="{{$sum.'a'}}"><br>
                   @endif
                   @endforeach
                   @endif
                   @endforeach
                 ...
                 <label for="{{$data->student->student_id.'p'}}" style="margin-left:20px;" {{request('student')==''}}>P</label>
                 <input type="radio" name="{{$data->student->student_id}}" value="present" id="{{$sum.'p'}}" require>
                <label for="{{$data->student->student_id.'a'}}">A</label>
                <input type="radio" name="{{$data->student->student_id}}" value="absent" id="{{$sum.'a'}}"><br>
                @endif
                  ...
                  @if($sum==0)
                <h2>No student added in this subject</h2>
                @endif
             ...
             @if($attendent==0)
             <h1>Attendent already taken by another subject edit</h1>
             @endif
