...
<table class="table">
   <tr>
    ...
    @if($student->class->coundelor_id == Auth::user()->id)
    <tr>
        <td>{{$student->name}}</td>
        ...
        <td><a href="{{'/edit_student/'.$student->student_id}}"><button type="button" class="btn btn-info">Edit</button></a>
        <a href="{{'delete_student/'.$student->student_id}}"><button type="button" class="btn btn-primary">Delete</button></a></td>
    </tr>
    @php
    $src=1;
    @endphp
