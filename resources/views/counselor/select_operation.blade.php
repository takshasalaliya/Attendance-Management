...
@endif
            ...
            <form action="selectes_data_counselor" method="get"> 
                @csrf
                ...
                <div class="col-md-6 mb-4 pb-2">
                  ...
                  <label class="form-label" for="subject" name="subject">Subject</label>
                    <select name="subject" id="subject" class="form-control form-control-lg">
                      ...
                      @php
                      $year=explode('/',$subject->subject->student_class->year);
                      $year1=explode('-',$year[0]);
                      $year2=explode('-',$year[1]);
                      @endphp
                      <option value="{{$subject->id}}" {{request('subject')==$subject->id?'selected':''}}>{{$subject->subject->short_name.'/'.$subject->subject->student_class->program->name.'/'.$subject->subject->student_class->sem.'/'.$subject->subject->student_class->devision.'/'.$year1[0].'-'.$year2[0]}}</option>
